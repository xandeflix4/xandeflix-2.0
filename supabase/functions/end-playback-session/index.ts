...
type EndPlaybackSessionRequest = {
  sessionId?: string;
  licenseCode?: string;
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    let payload: EndPlaybackSessionRequest;

    try {
      payload = (await request.json()) as EndPlaybackSessionRequest;
    } catch {
    ...
    const { data: session, error: sessionError } = await supabaseAdmin
      ...
      .eq('status', 'active')
      .select('id, device_identifier, status, ended_at, expires_at')
      .maybeSingle();
